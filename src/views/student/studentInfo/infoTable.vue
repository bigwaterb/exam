<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column label="编号" prop="sno" sortable align="center" width="140" v-if="false">
      ...
      <el-table-column prop="stuCreateTime" sortable label="注册时间" align="center" width="160">
        <template slot-scope="scope">
          ...
          <span>{{ scope.row.UserRegisterTime | date-format }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="登录权限" align="center" class-name="small-padding" width="120">
        <template slot-scope="{row}">
          ...
          <el-button v-waves v-if="row.UserState=='0'" size="mini" icon="el-icon-success" type="success" @click="handleModifyStatus(row,0)">
            启用状态
          </el-button>
          <!-- <el-button v-waves v-if="row.stuStatus=='0'" size="mini" icon="el-icon-error" type="danger" @click="handleModifyStatus(row,'1')"> -->
          <el-button v-waves v-if="row.UserState=='1'" size="mini" icon="el-icon-error" type="danger" @click="handleModifyStatus(row,1)">
            禁用状态
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="性别" prop="UserSex">
          ...
          <el-select v-model="temp.UserSex" class="filter-item">
            <el-option v-for="item in stuSexOptions" :key="item.key || 0" :label="item.label" :value="item.key" />
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      stuSexOptions: [{ label: '男', key: 0 }, { label: '女', key: 1 }],
      temp: {
        // sno: '',
        // stuPsw: '123456',
        // stuName: '',
      ...
      rules: {
        ...
        LoginName: [{ required: true, message: '学号为必填项', trigger: 'blur' },{ min: 12, max: 12, message: '学号必须为12位数字', trigger: 'blur' }],
        UserPwd: [{ required: true, message: '密码为必填项，默认设置密码为123456', trigger: 'blur' }],
  ...
  methods: {
    async getList() {
      ...
      if (result !== null){
        ...
        this.list = result.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
      }
      // 延迟0.5秒等待请求数据
      setTimeout(() => {
    ...
    async handleModifyStatus(row, status) {
      // row.UserState = int.parseInt(status)
      console.info(row)
